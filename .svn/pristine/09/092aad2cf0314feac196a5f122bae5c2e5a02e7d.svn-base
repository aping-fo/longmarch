...
//            nameValuePairs.add(new BasicNameValuePair(SIGN, sign));
    ...
    public static String sendPostRequestWithHeader(String url, String jsonContent, Map<String, String> header) throws Exception{
        CloseableHttpClient httpclient = HttpClients.custom().setDefaultRequestConfig(requestConfig).build();
        CloseableHttpResponse response = null;
        try {
            HttpPost httpPost = new HttpPost(url);
            ...
            if(httpEntity.getContentType().getValue().equals("application/json; charset=UTF-8")){
                String json = EntityUtils.toString(httpEntity);
                EntityUtils.consume(httpEntity);
                return json;
            }

            File file = new File("E:\\workspace\\wechat_game\\server\\wxacode\\1.png");
            if (!file.exists()){
                file.createNewFile();
            }

            OutputStream opstream = new FileOutputStream(file);
            httpEntity.writeTo(opstream);
